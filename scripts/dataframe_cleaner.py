"""
		This file will be responsible for data cleaning, like dropping duplicates, filling missing values, ..etc
		Number of classes : 1
...
class DfCleaner():
	...
	def columns_too_much_null(self, df: pd.DataFrame, percentage: int) -> pd.DataFrame:
		...
		for index, row in df.iterrows():
			if float(row["null_percentage"].replace("%", '')) > percentage:
